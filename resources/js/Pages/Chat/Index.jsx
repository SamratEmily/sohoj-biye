                    ...
                    <div className="space-y-4">
                        <div className="space-y-3">
                            {chatRooms.data.map((room) => (
                                <div
                                    key={room.id}
                                    className="glass-card p-5 flex items-center space-x-4 hover:border-primary-500/30 transition-all duration-300 group cursor-pointer relative"
                                    onClick={() => router.visit(`/chat/${room.id}`)}
                                >
                                    <Link 
                                        href={room.other_user.biodata_id ? `/feed/${room.other_user.biodata_id}` : '#'} 
                                    ...
                                    >
                                        ...
                                        {room.unread_count > 0 && (
                                            <span className="absolute -top-1 -right-1 w-5 h-5 bg-primary-500 rounded-full text-white text-xs flex items-center justify-center font-bold border-2 border-white dark:border-dark-900">
                                                {room.unread_count}
                                            </span>
                                        )}
                                    </Link>
                                    <div className="flex-1 min-w-0">
                                        <Link 
                                            href={room.other_user.biodata_id ? `/feed/${room.other_user.biodata_id}` : '#'} 
                                            onClick={(e) => e.stopPropagation()}
                                            className="relative z-10 font-semibold text-slate-900 dark:text-dark-100 hover:text-primary-500 dark:group-hover:text-primary-400 transition-colors inline-block mb-0.5"
                                        >
